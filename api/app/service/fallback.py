...
async def extract_with_fallback(
    ...
    """
    baseline_title, baseline_units, baseline_error = await _baseline(url)
    ...
    if baseline_error is None and _spoken_words(baseline_units) >= _FIRECRAWL_WORD_FLOOR:
        return baseline_title, baseline_units
    ...
    try:
        fc_title, fc_units = await run_in_threadpool(
...
async def _baseline(url: str) -> tuple[str | None, list[Unit], ExtractionError | None]:
    try:
        title, units = await run_in_threadpool(extract_article, url)
    except ExtractionError as e:
        return None, [], e
    return title, units, None


def _fetch_and_extract(url: str, api_key: str) -> tuple[str | None, list[Unit]]:
    ...
    try:
        return _extract_units_from_html(raw_html, url)
    except ExtractionError:

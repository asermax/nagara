...
This is discovery-mode: the *judgment* was fixed in advance; the *shape* is what the experiment discovered, by building several coherent whole-screen mockups and recombining them rather than committing to one design up front (see [[lab/README|the lab]]'s "explore several variants and recombine" method note).
...
The mockups answer *shape*, layout, navigation, usability, the mode structure, not visual design. The placeholder aesthetic (palette, type scale, spacing) carries no verdict weight; proper visual design is a separate, later idea (see [[read-along-visual-design]]).
...
Related: [[lab/README|the lab]] · [[read-along-player]] · [[read-along-timing]] · [[item-contract]] · [[markdown-paragraph-pipeline]]

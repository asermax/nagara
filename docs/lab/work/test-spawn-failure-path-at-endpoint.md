...
`chore`, not `defect`: read against the code, the branch is a straightforward mirror of its tested sibling and there is no reason to think it is wrong. What is missing is the regression test, not a fix.

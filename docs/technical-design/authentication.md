...
**Multi-user is a real replacement, not an extension.** Going multi-user means introducing genuine identity and per-user key management, replacing the single-key shim rather than adding to it. See [[auth]] in the work backlog for the OAuth slice this becomes, and [[queue|the queue]] for what a user sees once multiple keys and key management exist.

The poll route has no direct test asserting it rejects a missing key, unlike the other two item routes; see [[test-unauthenticated-poll-rejected]].

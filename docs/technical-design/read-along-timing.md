...
Every `ready` item carries a per-paragraph timing window alongside its text (see [item-contract](item-contract.md)), produced by the TTS service ([tts-service](tts-service.md)). The read-along player, not yet built in `web/`, consumes them, and caption export, which does not exist yet, will too. The pause-fold rule behind those windows applies wherever timing is produced or read.

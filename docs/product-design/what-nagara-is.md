...
> Building more of the product before knowing whether anyone wants the thing would be speculative work against an unanswered question. The public-funnel pieces (a login, article creation by a stranger, a landing page) exist to make that question answerable; the single-user pipeline does not need them.

## ⏩ What is not built yet, in order

The **dogfood group** hardens the single-user pipeline that already works: API hardening (quota, a list endpoint, key CRUD), then the queue view listing a user's items, then a settings page (voice default, key management). The **public-funnel group** turns the product into something a stranger can try, each piece built to answer the demand question: a login (Google OAuth plus a paste-then-login replay), then article creation by a signed-in stranger, then a landing page.

## 🚫 What is deliberately out of scope

- **File uploads** (PDFs, documents): a whole content-ingestion system of its own, not this product's.
...
- **AAC 48k audio fallback**: Opus ships first; this fallback is added only if pre-17 Safari support turns out to matter.

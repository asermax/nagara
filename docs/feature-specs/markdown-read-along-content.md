...
|----|-------------|--------|----------|
| R1 | A `ready` item's paragraphs carry a **display** form that preserves the article's markdown formatting, distinct from the **spoken** form that drives the audio. The display form is valid, renderable markdown: emphasis boundaries are repaired so a client never shows fused words or stray `**`/`*` markers | Core goal | [exp 002](../../experiments/002-markdown-paragraphs/README.md) |
...
| R5 | Paragraph segmentation matches the article's logical units: a paragraph is one display unit (its soft-wraps joined), each list item is its own unit, a code block is one atomic unit, and a blockquote or table block is one unit. Segmentation is robust to an inline code fragment the extractor mis-renders as a mid-paragraph fence — it stays inline in its paragraph rather than derailing the boundaries of the code blocks that follow | Must-have | [exp 002](../../experiments/002-markdown-paragraphs/README.md) |
...
- **R1** — Given a `ready` item whose article contains formatting, When it is read, Then each paragraph exposes a display form that retains the markdown (e.g. emphasis markers, a heading prefix, a link) and a distinct plain spoken form, and the two are individually observable; and the display markdown is valid — a run-in emphasis boundary (`**phrase**word`, `**text: **more`) renders as `**phrase** word` / `**text:** more`, never as fused words or leftover `**`/`*`.
...
- **R5** — Given article markdown, When it is segmented, Then a multi-line paragraph collapses to one unit (soft-wraps joined), a three-item list yields three units, a fenced code block stays a single unit with its internal newlines intact, and a blockquote or table block is a single unit — the unit boundaries match the article's logical structure, not raw line breaks; and Given an inline code fragment the extractor emits as a mid-paragraph fence, When it is segmented, Then that fragment collapses back to an inline code span so its paragraph stays one unit and the following fenced code block stays a separate atomic unit.

...
| R8 | Every route — enqueue, poll, and audio — requires authentication; the credential identifies the user, and a private item (including its audio) is never reachable without it | Must-have | [exp 001](../../experiments/001-player-ready-item/README.md) |
...
- **R3** — Given an item still generating, When it is polled, Then it reports `generating` (not an error and not a block); and When the underlying generation has crashed, Then a poll reports `failed` with a populated, human-readable error — the two are distinguishable from the item alone, with no retry/backoff bookkeeping required of the client. Generation failure surfaces either at enqueue (when it cannot even begin — see R4) or on a later poll (when a running job crashes); in both cases the terminal item carries the error.
...
- **R7** — Given a `ready` item, When its audio link is fetched with a valid credential, Then playable audio is streamed; and Given an item that is not `ready`, When its audio is fetched, Then audio is refused as not-available.
...
- **Browser audio delivery.** R8 requires a credential on the audio route, but a browser audio element cannot attach a custom credential header. Serving audio to an in-browser player needs a different mechanism (signed URLs or session/cookie auth). Resolve when the web player is built — captured in `BACKLOG.md` (auth-for-audio graduation).

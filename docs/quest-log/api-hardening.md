...
Furniture deliberately deferred when the backend spine was promoted, not yet built (the spine itself is in place): per-user quota enforcement (item-count tiers), a `GET /items` list endpoint, and API-key create/revoke (which also surfaces in the settings slice).
...
Related: [[quest-log/README|the quest log]] · [[validate-demand]]
